...
from agent.models import FlowDecoder_WOX1, WorldModelShared, FlowDecoderBidirectional, MotionDecoderWithPos, FlowDecoderRes, DecodeModel, WorldModelRes, FlowDecoder, IDM, IDM_NVQ, IDM_Continue, FlowAutoEncoder, Policy, Policy_one, WorldModel, WorldModel_Continue, SharedEncoder, Actor, Critic, Actor_Independent, Critic_Independent, FlowModel
...
def create_dynamics_models_flow_fae(
    model_cfg: config.ModelConfig, state_dicts: dict | None = None
) -> tuple:
    """
    v6 (flow-autoencoder) variant:
      - FlowAutoEncoder replaces IDM: obs_of[t] → z_t via reconstruction
      - WM and FM unchanged
    """
    wm_in_depth  = 3 * config.ADD_TIME_HORIZON
    wm_out_depth = 3

    fae = FlowAutoEncoder(
        z_dim=model_cfg.la_dim,
        in_channels=3,
        base_size=model_cfg.wm_scale,
    ).to(config.DEVICE)
    ...
    if state_dicts is not None:
        fae.load_state_dict(state_dicts["fae"])
        wm.load_state_dict(state_dicts["wm"])
        fm.load_state_dict(state_dicts["fm"])
...
def create_dynamics_models_flow_nvq(
    model_cfg: config.ModelConfig, state_dicts: dict | None = None
) -> tuple[IDM_NVQ, WorldModel, FlowDecoder_WOX1]:
    """
    v5 (no-VQ) variant: IDM_NVQ replaces IDM to avoid codebook collapse.
    WM and FM are identical to create_dynamics_models_flow.
    """
    obs_depth = 3
    idm_in_depth = obs_depth * (1 + config.ADD_TIME_HORIZON)
    wm_in_depth = obs_depth * (0 + config.ADD_TIME_HORIZON)
